...
Consumes:
    results/scores/matrices/{V_d_AR,V_d_MLM}.npy
...
def main() -> None:
    ...
    print(f"[input] panel {len(panel)} probes; matrix_metadata covers "
          f"{sum(1 for k in metadata if k.startswith('V_d_'))} V_d matrices")

    matrix_summaries: dict[str, dict] = {}
    for mat_name, mat_spec in metadata.items():
        if not mat_name.startswith("V_d_"):
            continue
    ...
    # of raw sum_log_p). We MUST use V, not V_d: V_d is double-centered so
    # column means are ~0 by construction and a Spearman / top-k on them
    # is just noise. V (clipped raw sum_log_p) preserves per-probe
    # difficulty.
    cross_out = out / "cross_branch"
    cross_out.mkdir(parents=True, exist_ok=True)
    cross_summary: dict = {"status": "skipped"}
    L_ar_path = in_dir / "matrices" / "V_AR.npy"
    L_mlm_path = in_dir / "matrices" / "V_MLM.npy"
    if (
        all(f"V_d_{b}" in matrix_summaries for b in ("AR", "MLM"))
...
def _write_report(
    ...
    lines.append("Generated by `scripts/run_phase1_analysis.py`. PCA + "
                 ...
                 "matrix outputs (V_d_AR / V_d_MLM) of "
    ...
    if cross_summary.get("status") == "ok":
        lines.append(
            f"- Spearman ρ between V_d_AR column means and V_d_MLM column means: "
