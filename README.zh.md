...
GLMap 是一个研究仓库,而非已发布的软件包——请克隆它并以可编辑模式安装。`pip install -e .` 会让 `glmap` 库可被 import(同时打通测试/脚本的导入),它**不是** PyPI 发行版。

```bash
git clone https://github.com/ai4nucleome/GLMap.git
cd GLMap
pip install -e .            # 核心:矩阵加载、分析、画图
pip install -e .[scoring]   # 额外:torch + transformers，用于模型打分
pip install -e .[dev]       # 额外:pytest
```

> **注意**:`import glmap` 不会触发 `import torch` 或 `import transformers`。重依赖在 `get_loader()` 内部按需加载,因此即使没有装 GPU 相关包,核心安装也足以用于分析和画图。
