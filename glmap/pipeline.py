"""Phase-1 scoring + matrix-aggregation pipeline (library layer).
...
embedding extractor, and the tests can import them directly:
  ...
  _score_ar_one_probe     score one probe with an AR loader
  _score_mlm_one_probe    score one probe with an MLM loader (stride PLL)
  _score_model            load a model + score the whole panel
  BranchMatrices          ModelMap L/Q/D triple for one branch
  _build_branch_matrices  aggregate per-model parquets -> V/V_d/D

Importing this module is heavy only insofar as it pulls the HF loader base
classes; it does NOT itself import torch (that happens lazily when a loader
is actually instantiated inside ``_score_model``).
"""
...
from glmap.loaders.huggingface import HFCausalLMLoader, HFMaskedLMLoader
