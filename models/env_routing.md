...
| `megadna` | — | — | — | latest | latest | `lingxusb/megaDNA` only. The upstream `megaDNA` package (+ its `MEGABYTE_pytorch` dep) needed to unpickle the `.pt` checkpoint is installed here, **not** in base — so megaDNA must route here even though it otherwise has base-like deps. |
...
lingxusb/megaDNA                            -> megadna       -> glmap.loaders.megadna.MegaDNALoader   megadna
